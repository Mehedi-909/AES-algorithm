...
void InvSubBytes()
{
	...
	for(i=0;i<4;i++)
	{
		for(j=0;j<4;j++)
		{
			stateArray[i][j] = getSBoxInvert(stateArray[i][j]);
...
void encryption()
{
	...
	for(round=1;round<numberOfRounds;round++)
	{
		...
		for(int m=0;m<4;m++){
            for(int n=0;n<4;n++){
               stateArray[m][n]=(unsigned char)mix[count];
                count++;
            }
		}
...
/*
	...
	*/
    ...
    cout<<"Plaintext in hexadecimal after decryption : "<<endl;
	for(i=0;i<numberOfWords*4;i++)
	{
		printf("%02x ",ciphertext[i]);
	}
